...
def train_model(console: Console, model_name: str, dataset_path: str, output_dir: str, is_mini_run: bool, learning_rate: float, lora_rank: int):
    ...
    # This function expects the dataset_path to point to a file with the 'text' field.
    # The pre-flight checks are now done before this function is ever called.
    with open(dataset_path, 'r', encoding='utf-8') as f:
        # The data is already formatted, no need to validate against TrainingPair here.
        trainer_data = json.load(f)
    hf_dataset = Dataset.from_list(trainer_data)
    console.print(f"Loaded [magenta]{len(hf_dataset)}[/magenta] formatted samples into Hugging Face Dataset.")
    ...
    lora_config = LoraConfig(r=lora_rank, lora_alpha=lora_rank * 2, target_modules=["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"], lora_dropout=0.05, bias="none", task_type="CAUSAL_LM")
    console.print(f"LoRA configured with rank=[yellow]{lora_rank}[/yellow] and alpha=[yellow]{lora_rank * 2}[/yellow].")
    training_args = TrainingArguments(output_dir=output_dir, per_device_train_batch_size=1, gradient_accumulation_steps=4, optim="paged_adamw_8bit", logging_steps=10, learning_rate=learning_rate, fp16=False, bf16=True, max_steps=50 if is_mini_run else 1000, warmup_ratio=0.03, group_by_length=True, lr_scheduler_type="constant", report_to="none")
    ...
    trainer = SFTTrainer(model=model, train_dataset=hf_dataset, peft_config=lora_config, dataset_text_field="text", max_seq_length=2048, tokenizer=tokenizer, args=training_args)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_dataset", type=str, required=True, help="Path to the TRAINER-FORMATTED training set (with 'text' field).")
    parser.add_argument("--preflight_train_dataset", type=str, required=True, help="Path to the ORIGINAL training set for pre-flight validation.")
    ...
    if run_preflight_checks(console, args.preflight_train_dataset, args.test_dataset):
        train_model(console, args.model_name, args.train_dataset, args.output_dir, args.is_mini_run, args.learning_rate, args.lora_rank)